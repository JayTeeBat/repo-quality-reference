# Changelog

All notable changes to this project are documented here. The project follows
semantic versioning once the first stable release is published.

## Unreleased

### Added

- Initial repository quality standard.
- Profiles for documentation-only, Python single-package, and Python workspace
  repositories.
- Self-auditing `repo-quality check` CLI.
- Golden-reference CI, tests, contributor guidance, and security policy.
- Executable documentation formatting, link-checking, and site-build commands.

### Changed

- Made README and `AGENTS.md` section requirements optional and
  repository-owned through `[documentation]`.
- Kept universal document checks limited to presence, non-empty content, and
  valid internal links.
